...
PULL REQUESTS. Open the PR as a draft, then WATCH it until it is merged or closed: subscribe to its activity, fix red CI and branch conflicts at once without asking, keep a check-in scheduled while it is open (Cem, 2026-09-05: "why dont you subscribe and resolve test fails and branch conflicts? these should be done automatically"). Cem merges his own PRs unless he says merge. Before every push, read the PR's STATE (open, merged, closed), not only its checks: a merged PR is finished, and a push to its branch is a push to nothing. Anything after the merge restarts from the default branch on the same branch name and gets a new draft PR; the prepush-merged guard blocks the common case, the rule covers the rest.
