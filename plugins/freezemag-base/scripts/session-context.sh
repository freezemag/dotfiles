...
MODELS. The main conversation is for judgment. Delegate file sweeps, greps, catalogue reads and test runs to the freezemag-base surveyor or test-runner agents (haiku); screenshot audits and doc-drift checks to screenshot-auditor and drift-checker (sonnet); adversarial review to reviewer (opus); simplifying a finished diff, on request only, to code-simplifier (opus). Give the built-in Explore agent model haiku explicitly.

PULL REQUESTS. After every push to a branch that is not the default branch, a draft PR must cover it: open one if none is open (the postpush-pr guard holds all work until create_pull_request or subscribe_pr_activity has run). Subscribe to the PR's activity and act on what arrives: fix red CI, branch conflicts and review comments at once, without asking. No hourly check-ins: while a PR is open keep ONE check-in scheduled 24 hours out (send_later, 1440 minutes), re-armed silently if nothing changed (Cem, 2026-09-06). Cem merges his own PRs unless he says merge. Before every push, read the PR's STATE (open, merged, closed), not only its checks: a merged PR is finished, and a push to its branch is a push to nothing; anything after the merge restarts from the default branch on the same branch name and gets a new draft PR (the prepush-merged guard blocks the merge-commit case).
